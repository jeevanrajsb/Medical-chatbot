...
import os, sys
from app.components.image_model import analyze_xray
    ...
    <div style='display: flex; justify-content: center; align-items: center; gap: 15px;'>
        ...
        <div>
            ...
            <p style='font-size: 1.0rem; color: #b2bec3; margin: 0; text-align: left; font-weight: 500;'>
                Advanced Medical QA using Retrieval Augmented Generation & Llama 3 and image analysis with TensorFlow models
            </p>
        </div>
    </div>
</div>
...
with tab1:
    ...
    <div style='background: linear-gradient(135deg, rgba(40, 116, 240, 0.1) 0%, rgba(155, 89, 182, 0.1) 100%); 
        ...
        <h3 style='color: #00d4ff; margin: 0 0 10px 0;'>🩺AI Medical Assistant Workspace</h3>
    ...
    with chat_container:
        for idx, message in enumerate(st.session_state.messages):
            ...
            display_name = "You" if message["role"] == "user" else " AI Medical Assistant"
    ...
    input_col, btn_col,upload_col,export_col, = st.columns([5, 5, 2,2])
    ...
    #             st.rerun()
        ...
        with btn_col:
            send_col, analyze_col, brain_col,clear_col = st.columns(4)

            with send_col:
                send_clicked = st.button("📤 Send", use_container_width=True, type="primary")

            with analyze_col:
                analyze_clicked = st.button("🩻Analyze X-ray", use_container_width=True,type="primary")

            with brain_col:
                analyze_brain_clicked = st.button(
                    "🧠 Brain MRI",
    ...
    image_label = None
    image_score = None
if analyze_brain_clicked:

    uploaded_file = st.session_state.get("uploaded_file", None)

    if uploaded_file is None:
        st.warning("⚠️ Please upload MRI image")
    else:
        from app.components.brain_model import analyze_brain

        label, score = analyze_brain(uploaded_file)
        ...
        st.success(f"🧠 {result} (Confidence: {score:.2f})")
# ================= IMAGE FLOW =================
if analyze_clicked:
    ...
    if uploaded_file is None:
        st.warning("⚠️ Please upload an X-ray first.")
    else:
        from app.components.image_model import analyze_xray

        try:
            image_label, image_score = analyze_xray(uploaded_file)

            if image_label == "PNEUMONIA" and image_score > 0.80:
                result = "Yes, pneumonia is detected."
            elif image_score < 0.60:
                result = "Result is uncertain."
            else:
            ...
            st.session_state.messages.append({
                "role": "assistant",
                "content": f"🩻 X-ray Result: {result} (Confidence: {image_score:.2f})"
            })

            st.write(f"🩻 {result} (Confidence: {image_score:.2f})")

        except Exception as e:
            st.error(f"❌ Image analysis failed: {e}") 

# 🔥 HANDLE QUICK BUTTON QUERY
if "pending_query" in st.session_state:
    ...
    if chain:
        with st.status("🧠 AI Medical Analysis...", expanded=True):
...
#     dd_tab1, dd_tab2, dd_tab4 = st.tabs(["🏗️ High-Level Architecture", "⚙️ RAG Engine Internals", "🔮 Future Roadmap"])
...
#         doc_tab1, doc_tab2, doc_tab3, doc_tab5 = st.tabs([
#             "🏗️ core Architecture", 
#             "🔄 Data & RAG Pipeline", 
#             "💻 Backend & Frontend", 
